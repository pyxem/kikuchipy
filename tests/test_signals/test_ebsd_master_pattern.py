...
class TestFitPatternDetectorOrientation:
    ...
    ref = ReciprocalLatticeVector(phase=phase, hkl=pqr_p)
    ref = ref.symmetrise()
    ...
    def setup_detector_sim_and_u_os(self, tilt, azimuthal, euler_2):
        ...
        euler_2 angles, get the GeometricalKikuchiPatternSimulation from
        the simulator and calculate the orientation matrix
        transforming from the detector reference frame CSd to
        the crystal cartesian reference frame CSc. Return these
        three items.
        ...
        euler_2 : Float
            The detector Euler angle phi2 (EBSDDetector.euler[2]) in deg.
        """
        det = self.detector.deepcopy()
        det.euler = np.array([azimuthal, 90 + tilt, euler_2])

        sim_lines = self.simulator.on_detector(det, self.rotations)

        u_os = np.matmul(self.u_o, det.u_s)
    ...
    def setup_za_and_cds(self, zone_axes, sim_lines, det):
        ...
        conversions = get_coordinate_conversions(det)
        cds_px = convert_coordinates(cds_gn, "gn_to_pix", conversions)

        cds_px_int = np.around(cds_px, decimals=0).astype(int)
    ...
    def get_d_ang(self, cds_px_int, conversions, u_os, d_vec_n, det):
        ...
        cds_gn_int = convert_coordinates(cds_px_int + 0.5, "pix_to_gn", conversions)
        vecs = np.hstack(
            (
                cds_gn_int * det.pcz,
    ...
    def get_r_ang_and_n_ang(self, cds_px_int, det, d_vec_n, dddd):
        ...
        r_g_array_rot = _rotate_vector(self.rotations.data[0], r_g_array)
    ...
    def calculate_fit(self, tilt, azimuthal, euler_2, zone_axes):
        """
        ...
        n_ang : np.ndarray
            ...
            match.
        """
        det, sim_lines, u_os = self.setup_detector_sim_and_u_os(
            tilt, azimuthal, euler_2
        )
    ...
    @pytest.mark.parametrize(
        "tilt, azimuthal, euler_2, zone_axes",
        [
    ...
    def test_fit_detector_orientation(self, tilt, azimuthal, euler_2, zone_axes):
        """
        ...
        """
        angles = self.calculate_fit(tilt, azimuthal, euler_2, zone_axes)
...
def get_coordinate_conversions(detector):
    ...
    "pix_to_gn" and "gn_to_pix". Under each of these keys
    is a further dict with the keys: "m_x", "c_x", "m_y"
    and "c_y". These are the slope (m) and y-intercept (c)
    corresponding to y = mx + c, which describes the
    conversion of the coordinates from pixels to
    gnomonic or vice versa.

    """
    m_pix_to_gn_x = (
        detector.gnomonic_bounds[..., 1] - detector.gnomonic_bounds[..., 0]
    ) / (detector.bounds[1] + 1)
    c_pix_to_gn_x = deepcopy(detector.gnomonic_bounds[..., 0])

    m_pix_to_gn_y = (
        detector.gnomonic_bounds[..., 2] - detector.gnomonic_bounds[..., 3]
    ) / (detector.bounds[3] + 1)
    c_pix_to_gn_y = deepcopy(detector.gnomonic_bounds[..., 3])
...
def convert_coordinates(
    ...
    """
    Convert coordinates from gnomonic to pixels or from pixels
    to gnomonic.
    ...
    coords_out
        ...
        or is the naviagation shape then the shape of the input.

    """
    coords_out = np.zeros_like(coords, dtype=np.float32)

    coords_out[..., 0] = (
        conversions[direction]["m_x"] * coords[..., 0] + conversions[direction]["c_x"]
    )
    coords_out[..., 1] = (
        conversions[direction]["m_y"] * coords[..., 1] + conversions[direction]["c_y"]
    )
    return coords_out

...
class TestIO:
        ...
        if 'complex' in dtype.name:
            with pytest.raises(ValueError, match='Data type'):
                kp.io._io._assign_signal_subclass(
                    dtype=dtype, signal_dimension=signal_dimension,
                    signal_type=signal_type, lazy=lazy)
    ...
    def test_save_extensions(self, extension):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if extension == '':
                s.save(file_path)
                assert os.path.isfile(file_path + '.h5') == True
            else:  # extension == '.h4'
    ...
    def test_save_to_existing_file(self, save_path_h5ebsd):
        ...
        with pytest.raises(ValueError, match='overwrite parameter can'):
            s.save(
                save_path_h5ebsd, scan_number=2, overwrite='False',
                add_scan=False)
        s.save(
            save_path_h5ebsd, scan_number=2, overwrite=False, add_scan=False)

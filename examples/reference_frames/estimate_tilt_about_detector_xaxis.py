...
normal (but in the opposite direction) :cite:`winkelmann2020refined`. The estimate is
found using :meth:`~kikuchipy.detectors.EBSDDetector.estimate_xtilt` which performs
linear regression of :attr:`~kikuchipy.detectors.EBSDDetector.pcz` vs.
:attr:`~kikuchipy.detectors.EBSDDetector.pcy`.

To test the estimation, we create some realistic projection center (PC) values (PCx,
PCy, PCz) by extrapolating from a PC in the upper left corner of a map, assuming a
nominal sample tilt of 70 degrees, a detector tilt of 0 degrees, a detector pixel size
of 70 microns and a sample step size of 50 microns.
"""
...
# Add ten outliers by adding more noise
